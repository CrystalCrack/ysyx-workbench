...
#define log_write(...) IFDEF(CONFIG_TARGET_NATIVE_ELF, do { extern FILE* log_fp; extern bool log_enable(); if (log_enable() && log_fp != NULL) { fprintf(log_fp, __VA_ARGS__); fflush(log_fp); } } while (0) )
...
#define INSTPAT_MATCH(s,name,type,...) { int rd = 0; word_t src1 = 0, src2 = 0, imm = 0; decode_operand(s, &rd, &src1, &src2, &imm, concat(TYPE_, type)); __VA_ARGS__ ; }
