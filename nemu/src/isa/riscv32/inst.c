...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai    , I, Assert((imm&0x20)==0, "srai: imm[5] must be 0"); R(rd) = (sword_t)src1 >> imm); //shift right arithmetic immediate
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge     , B, if(src1 > src2) s->dnpc = s->pc + imm);// Branch if Greater or Equal
